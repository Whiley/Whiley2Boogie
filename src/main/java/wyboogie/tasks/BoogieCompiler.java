...
public class BoogieCompiler extends AbstractTranslator<Decl, Stmt, Expr> {
    ...
    public BoogieCompiler(BoogieFile target) {
        super(subtyping);
    ...
    /**
     * Enable of disable the use of structured loops.
     * @param flag
     */
    public void setStructuredLoops(boolean flag) {
        this.structuredLoops = flag;
    }
    ...
    private List<Decl> constructLambdaAxioms(String name, WyilFile.Type param, WyilFile.Type ret, Tuple<WyilFile.Template.Variable> template, Syntactic.Item item) {
    ...
    public List<Decl.Variable> constructLocals(WyilFile.Stmt blk) {
        ...
        new AbstractFold<Boolean>() {
            ...
            public Boolean constructAssign(WyilFile.Stmt.Assign stmt, List<Boolean> lvals, List<Boolean> rvals) {
                if (!WyilUtils.isSimple(stmt) && WyilUtils.hasInterference(stmt)) {
            ...
            @Override
            public Boolean constructEqual(WyilFile.Expr.Equal expr, Boolean lhs, Boolean rhs) {
                WyilFile.Type lhsT = expr.getFirstOperand().getType();
                Boolean r = join(lhs,rhs);
                // For tuple comparisons, a logical and will be used to physically implement the comparison.
                if(r && lhsT.shape() > 1) {
            ...
            @Override
            public Boolean constructNotEqual(WyilFile.Expr.NotEqual expr, Boolean lhs, Boolean rhs) {
                WyilFile.Type lhsT = expr.getFirstOperand().getType();
                Boolean r = join(lhs,rhs);
                // For tuple comparisons, a logical and will be used to physically implement the comparison.
                if(r && lhsT.shape() > 1) {
    ...
    private List<Decl> constructStandaloneLambda(WyilFile.Decl.Lambda l) {
        ...
        Set<WyilFile.Decl.Variable> captured = l.getCapturedVariables();
        //
        ...
        Pair<List<Decl.Parameter>, List<Expr.Logical>> capturedParametersAndConstraints = constructParameters(null, new Tuple<>(l.getCapturedVariables()), HEAP);
    ...
    public Stmt constructDoWhile(WyilFile.Stmt.DoWhile stmt, Stmt body, Expr condition, List invariant) {
        ...
        final String CONTINUE_LAB = "CONTINUE_" + stmt.getIndex();
        final String BREAK_LAB = "BREAK_" + stmt.getIndex();
        ArrayList<Stmt> stmts = new ArrayList<>();
        // Add first iteration
        stmts.add(body);
        // Fresh all labels used within the body.  This is necessary since we are duplicating the body and, hence,
        // this duplicates any labels contained therein.  Observe that those used for break and continue in *this* loop
        // must be ignored.
        body = freshenLabelsExcept(body, BREAK_LAB, CONTINUE_LAB);
        // Construct the loop itself
        stmts.add(constructLoop(stmt, condition, invariant, body));
        // Done
        return SEQUENCE(stmts);
    }
    ...
    public Stmt constructIndirectInvokeStmt(WyilFile.Expr.IndirectInvoke expr, Expr source, List<Expr> args) {
        ...
        if (ft instanceof WyilFile.Type.Property) {
            ...
            // Extract any holes
            Tuple<WyilFile.Template.Variable> holes = WyilUtils.extractTemplate(ft);
            // Determine lvals. Observe that we have to assign any return values from the
            // procedure called, even if they are never used.
            List<LVal> lvals = new ArrayList<>();
            if (type.shape() == 1) {
    ...
    public Stmt constructWhile(WyilFile.Stmt.While stmt, Expr condition, List invariant, Stmt body) {
        return constructLoop(stmt, condition, invariant, body);
    }

    private Stmt constructLoop(WyilFile.Stmt.Loop stmt, Expr _condition, List<Expr.Logical> invariant, Stmt body) {
        final String HEAD_LAB = "HEAD_" + stmt.getIndex();
        final String BODY_LAB = "BODY_" + stmt.getIndex();
        final String CONTINUE_LAB = "CONTINUE_" + stmt.getIndex();
        final String BREAK_LAB = "BREAK_" + stmt.getIndex();
        final String EXIT_LAB = "EXIT_" + stmt.getIndex();
        final boolean needContinueLabel = containsContinueOrBreak(stmt, false);
        final boolean needBreakLabel = containsContinueOrBreak(stmt, true);
        final boolean pure = isPure(stmt);
        Tuple<WyilFile.Decl.Variable> modified = stmt.getModified();
        ArrayList<Stmt> stmts = new ArrayList<>();
        // Flattern (potentiall impure) condition
        Pair<List<Stmt>,Expr> f = flatternImpureExpression(_condition);
        // Extract side-effects
        List<Stmt> sideEffects = f.first();
        Expr.Logical condition = (Expr.Logical) f.second();
        // Ensure invariant is well-defined
        invariant = flatternAsLogical(invariant);
        // Add any type constraints arising (first)
        invariant.addAll(0, constructTypeConstraints(modified, HEAP));
        if (!pure) {
            // Variable to store OLD heap
            Expr.VariableAccess OHEAP = BoogieFile.VAR("HEAP$" + stmt.getIndex());
            // Save OLD heap
            stmts.add(BoogieFile.ASSIGN(OHEAP, HEAP));
            // Add necessary loop frame
        ...
        // FIXME: could do better here if all "side effect" statements are actually assertions.
        if(structuredLoops && sideEffects.size() == 0) {
            // Add continue label (if necessary)
            if (needContinueLabel) {
                stmts.add(LABEL(CONTINUE_LAB));
            }
            ...
            stmts.add(LABEL(BODY_LAB));
            stmts.addAll(sideEffects);
            // Assume Loop condition
            stmts.add(ASSUME(condition));
            stmts.add(body);
            ...
            stmts.add(ASSUME(NOT(condition)));
        }
        if (needBreakLabel) {
            stmts.add(LABEL(BREAK_LAB));
        }
        //
        return SEQUENCE(stmts);
    }
...
//        return REM(lhs, rhs, ATTRIBUTE(expr));
    ...
    public Pair<List<Stmt>, Expr> flatternImpureExpression(Expr e) {
        ...
        stmts.addAll(extractor.visitExpression(r.second()));
        // Done
...
//                OR(EQ(v, VOID), EQ(INVOKE("Array#Length", a), INVOKE("Array#Length", PUT(a, i, v)))))));
    ...
    private List<Decl> constructFunctionOrMethodLambda(WyilFile.Type.Callable mt) {
        ...
        Tuple<WyilFile.Template.Variable> holes = WyilUtils.extractTemplate(mt);
        //
    ...
    private List<Decl> constructMetaTypeAxioms(WyilFile f) {
        ...
        for (WyilFile.Type type : types) {
            // Extract all holes
            Tuple<WyilFile.Template.Variable> holes = WyilUtils.extractTemplate(type);
            // Act accordingly
     ...
     * @return
     */
    private Expr.Logical constructTypeConstraint(WyilFile.Type type, Expr expr, Expr heap, Syntactic.Item item) {
        switch (type.getOpcode()) {
            case WyilFile.TYPE_null:
            case WyilFile.TYPE_bool:
            case WyilFile.TYPE_byte:
     ...
     * @return
     */
    private Expr.Logical constructNominalTypeTest(WyilFile.Type.Nominal to, WyilFile.Type from, Expr argument, Expr heap, Syntactic.Item item) {
        ArrayList<Expr> arguments = new ArrayList<>();
    ...
    public Expr.Logical constructUniversalTypeTest(WyilFile.Type.Universal to, WyilFile.Type from, Expr argument, Expr heap, Syntactic.Item item) {
     ...
     * @return
     */
    private Expr.Logical constructRecordTypeTest(WyilFile.Type.Record to, WyilFile.Type from, Expr argument, Expr heap, Syntactic.Item item) {
     ...
     * @return
     */
    private Expr.Logical constructArrayTypeTest(WyilFile.Type.Array to, WyilFile.Type from, Expr argument, Expr heap, Syntactic.Item item) {
    ...
    private Expr.Logical constructReferenceTypeTest(WyilFile.Type.Reference to, WyilFile.Type from, Expr argument, Expr heap, Syntactic.Item item) {
     ...
     * @return
     */
    private Expr.Logical constructUnionTypeTest(WyilFile.Type.Union to, WyilFile.Type from, Expr argument, Expr heap, Syntactic.Item item) {
    ...
    private Expr.Logical constructFunctionTypeTest(WyilFile.Type.Function to, WyilFile.Type from, Expr argument, Expr heap, Syntactic.Item item) {
    ...
    private Expr.Logical constructMethodTypeTest(WyilFile.Type.Method to, WyilFile.Type from, Expr argument, Expr heap, Syntactic.Item item) {
        // Ensure lambda argument unboxed
        Expr nArgument = cast(to,from,argument);
        // Construct type test (when necessary)
        return nArgument != argument ? INVOKE("Lambda#is", argument, ATTRIBUTE(item)) : CONST(true);
     ...
     * @return
     */
    private Expr.Logical within(WyilFile.Type type, Expr p, Expr q, Expr heap, Syntactic.Item item) {
    ...
    private Expr.Logical within(Expr r, Tuple<WyilFile.Decl.Variable> variables, Expr heap, Syntactic.Item item) {
    ...
    private Expr.Logical within(WyilFile.Type type, Expr r, Function<Integer,String> parameters, Expr heap, Syntactic.Item item) {
        ArrayList<Expr.Logical> clauses = new ArrayList();
        for(int i=0;i!=type.shape();++i) {
    ...
    private List<Expr.Logical> constructMethodFrame(WyilFile.Type.Callable ct, Function<Integer,String> parameters, Function<Integer,String> returns, Syntactic.Item item) {
    ...
    /**
     * Determine list of variables which should be "havoced" in a loop.
     * @param loop
     * @return
     */
    private List<String> determineHavocs(WyilFile.Stmt.Loop loop) {
        Tuple<WyilFile.Decl.Variable> modified = loop.getModified();
        ArrayList<String> havocs = new ArrayList<>();
        for (int i = 0; i != modified.size(); ++i) {
            havocs.add(toVariableName(modified.get(i)));
        }
        if(!isPure(loop)) {
            havocs.add("HEAP");
        }
     ...
     */
    private static WyilFile.Decl.Variable extractVariable(WyilFile.LVal lval) {
        // FIXME: this should really be a query over e.g. lval.getDescendant().
        switch (lval.getOpcode()) {
            case WyilFile.EXPR_arrayaccess:
            case WyilFile.EXPR_arrayborrow: {
                WyilFile.Expr.ArrayAccess e = (WyilFile.Expr.ArrayAccess) lval;
                return extractVariable((WyilFile.LVal) e.getFirstOperand());
            }
            case WyilFile.EXPR_fielddereference: {
                WyilFile.Expr.FieldDereference e = (WyilFile.Expr.FieldDereference) lval;
                return extractVariable((WyilFile.LVal) e.getOperand());
            }
            case WyilFile.EXPR_dereference: {
                WyilFile.Expr.Dereference e = (WyilFile.Expr.Dereference) lval;
                return extractVariable((WyilFile.LVal) e.getOperand());
    ...
    private boolean isPure(WyilFile.Stmt s) {
        ...
        boolean pure = new AbstractVisitor() {
            boolean result = true;
    ...
    private static String freshLabel(String prefix) {
        return prefix + "_" + (LABEL_COUNTER++);
    }

    /**
     * Freshen all the labels in a given statement.  This basically makes sure that all labels are unique.
     * @param stmt
     * @return
     */
    public static Stmt freshenLabelsExcept(Stmt stmt, String... exceptions) {
        HashMap<String,String> mapping = new HashMap<>();
        // Initialise mapping with exceptions
        for(String s : exceptions) {
            mapping.put(s,s);
        }
        //
        return new AbstractStatementVisitor() {
            @Override
            protected Stmt constructGoto(Stmt.Goto stmt) {
                List<String> labs = map(stmt.getLabels(), l -> get(l));
                return GOTO(labs);
            }
            @Override
            protected Stmt constructLabel(Stmt.Label stmt) {
                String lab = get(stmt.getLabel());
                return LABEL(lab);
            }

            private String get(String lab) {
                String nlab = mapping.get(lab);
                if(nlab == null) {
                    nlab = freshLabel(lab);
                    mapping.put(lab,nlab);
                }
                return nlab;
            }
        }.visitStatement(stmt);
